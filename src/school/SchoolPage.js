...
const SchoolPage = ({ school = {}, deleteSchool, updateSchool, error, setError}) => {
  if (!school.id) {
    return <p> Loading... </p>;
  }
    ...
    <div className="flex-1 mx-4">
      <form className="bg-white shadow-md rounded-lg px-8 pt-6 pb-8 mb-4" onSubmit={onSubmit}>
        ...
        <div>
          ...
          <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 mx-4 rounded" type="button" onClick={() => {history.push('/'); setError('');}} >Cancel</button>
        </div>
      </form>
    </div>
  );
